...
var dp_end=$("#dp_end").val();
		...
		var excel = path+'/ExporttoExcel_8.jsp?uid=<%=session_vid_resouce%>&fdate='+dp_start+'&tdate='+dp_end+'&sub_series_list='+sub_series_list+'&sname='+sname;
...
$.ajax({
			type : "post",
			url : path + "/Fetching_egt_Series_report_lazyload",
			async : false,
			...
			success : function(h) {
				...
				if(sub_series_list=="<%=series_name_8_1%>"){
				 ...
				 cond=cond+'<tr><th class="text-center">S.No.</th><th class="text-center">Calling Parties Number</th><th class="text-center">Destination Party Number</th><th class="text-center">Calling Time</th><th class="text-center">Ans Time</th><th class="text-center">Date</th><th class="text-center">Call Ring Time(Secs)</th> </tr></thead><tbody>';
				}
				else if(sub_series_list=="<%=series_name_8_2%>"){
					...
					cond='<table class="table table-striped table-bordered table-condensed display nowrap"	id="reportdata" width="100%"><thead><tr id="msgcnt"><th colspan="5" ><div id="rowpintdiv" style="font-weight:bold;"><div class="recordscnt">Total Records - <span id="passport_row_totcnt"></span>/<span id="passport_row_cnt"></span></div></th></tr>';
					 cond=cond+'<tr><th class="text-center">S.No.</th><th class="text-center">Date</th><th class="text-center">Total Call</th><th class="text-center">Lost call</th><th class="text-center">Lost Percentage</th> </tr></thead><tbody>';
					}
				else {
				...
				$.each(j,function(a, b) {
					...
					if(sub_series_list=="<%=series_name_8_1%>"){
				cond=cond+"<tr><td style='text-align:center;'>"+b.rownumber+"</td><td  style='text-align:center;'>"+b.calling_parties_number+"</a></td><td  style='text-align:center;'>"+b.destination_party_number+"</td><td style='text-align:center;'>"+b.calling_time+"</td><td style='text-align:center;'>"+b.Ans_time+"</td><td style='text-align:center;'>"+b.dt+"</td><td style='text-align:center;'>"+b.call_ring_time+"</td></tr>";
				
				successavg=parseInt(successavg)+parseInt(b.call_ring_time);
				
				succtotavg= b.Total_Average;
				
					
					}
					else if(sub_series_list=="<%=series_name_8_2%>"){
						cond=cond+"<tr><td style='text-align:center;'>"+b.rownumber+"</td><td>"+b.dt+"</a></td><td  style='text-align:center;'>"+b.total_call+"</td><td style='text-align:center;'>"+b.lost_call+"</td><td style='text-align:center;'>"+b.lost_percentage+"</td></tr>";

						successavg=parseInt(successavg)+parseInt(b.total_call);
						failureavg=parseInt(failureavg)+parseInt(b.lost_call);
						 
						succtotavg= b.Total_total_call;
						failtotavg= b.Total_lost_call;
					}
				...
				vahan_rowrecord=b.rownumber; 
					...
					cond=cond+" <tfoot>            <tr>                <th colspan='6' style='text-align:right'>Average:</th>                <th id='successavg' style='text-align:right;font-weight:bold;'></th>            </tr>        </tfoot>";
					
					successavg=parseFloat(successavg)/parseFloat(i);
					...
					cond=cond+" <tfoot>            <tr>                <th colspan='2' style='text-align:right'>Total:</th>                <th id='successavg' style='text-align:right;font-weight:bold;'></th>  <th id='failureavg' style='text-align:right;font-weight:bold;'></th>    <th></th>        </tr>        </tfoot>";
					
					successavg=parseFloat(successavg)/parseFloat(i);
					successavg=parseFloat(successavg.toFixed(2));
					
					failureavg=parseFloat(failureavg)/parseFloat(i);
			...
			$("#exporttoexcel").css("display","none");
					...
					if(sub_series_list=="<%=series_name_8_1%>"){
						$("#successavg").html(successavg+"(Overall Percentage  - " +succtotavg+")");
					}
					else if(sub_series_list=="<%=series_name_8_2%>"){
						$("#successavg").html(successavg+"(Overall Total  - " +succtotavg+")");
						$("#failureavg").html(failureavg+"(Overall Total  - " +failtotavg+")");
